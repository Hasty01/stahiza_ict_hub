    ...
    try {
      if (formMode === 'register') {
        const user = await registerUser({
          email: formData.email || `${formData.username}@stahiza.internal`,
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {formMode === 'login' ? (
                <div className="space-y-1.5">
                  <label className="text-[9px] uppercase font-bold tracking-widest text-slate-400 ml-1">Username or Email</label>
                  <input 
                    type="text" 
                    name="email"
                    required
              ...
              {formMode === 'register' && (
                <>
                  <div className="space-y-1.5">
                    <label className="text-[9px] uppercase font-bold tracking-widest text-slate-400 ml-1">Class / vClass</label>
                    <input 
                      type="text" 
                      name="vclass"
                      required
                      placeholder="S.4 Science"
                      className="w-full h-11 bg-black/40 border border-white/10 rounded-xl px-4 text-xs focus:border-cyan-500 transition-all outline-none"
